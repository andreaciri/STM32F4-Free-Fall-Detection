...
void Lis3dsh::config(const char* purpose)
{
    ...
    if (strcmp(purpose, "freefall")==0)
    {
        ...
        toSend[0] = SETT1;
        toSend[1] = SETT1_SITR; //STOP and CONT commands generate an interrupt and perform output actions as OUTC command
        toSend[1] |= SETT1_R_TAM; //NEXT condition validation : standard mask always evaluated 
        spi.writeAndRead(toSend, write);
